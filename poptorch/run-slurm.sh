# This is the script to be called by job.sh
...
MPI_SETTINGS="--mpi-global-args='--tag-output --allow-run-as-root --mca oob_tcp_if_include "$NETMASK" --mca btl_tcp_if_include "$NETMASK" --npernode "$SLURM_NTASKS_PER_NODE" ' \
	--mpi-local-args=' -x OPAL_PREFIX -x LD_LIBRARY_PATH -x PATH -x PYTHONPATH -x IPUOF_VIPU_API_TIMEOUT=600 -x POPLAR_RUNTIME_OPTIONS -x TEMPDIR -x TMPDIR -x TEMP -x TMP --npernode "$SLURM_NTASKS_PER_NODE" ' \
